...
class HavaleMiktar extends Component {
    constructor(props){
        super(props)
        this.state={
            Miktar:''
...
transfer = (GondericiNo,AliciNo)=>{
    ...
    fetch('https://bankappapi.azurewebsites.net/api/Hesap/Havale?GonderenNo='+GondericiNo+'&AliciNo='+AliciNo+'&Miktar='+this.state.Miktar, 
     ...
     headers: {
       ...
       .then((res) => {
        if(res!=null)
                Alert.alert("Para Transferi Gerçekleşti")
            })
       .catch((error) => {     
            console.log(error)
       });
           ...
           <View style={styles.container}>
               ...
               <View style={styles.containerStyle}>
                    <View style={styles.subContainerStyle}>
                        <View >
                            <Text style={{ fontSize: 18 }}>{getParam('Bakiye')}</Text>
                ...
                <View style={styles.button}>
                    ...
                    <Button onPress={() => this.transfer(getParam('HesapNo'),getParam('AliciHesapNo'))}  
                    color='#c0392b' 
